<template>
  <div class="p-4 sm:p-6 lg:p-8 mt-8 sm:mt-12 lg:mt-16">
    <h1 class="text-2xl sm:text-3xl lg:text-4xl font-bold text-red11 mb-4 text-center">Contact</h1>
    <p class="text-gray11 mb-6 sm:mb-8 lg:mb-10 text-center text-sm sm:text-base lg:text-lg font-normal max-w-2xl mx-auto">
      {{ description }}
    </p>
    <div class="space-y-4 sm:space-y-5 lg:space-y-6 max-w-xl mx-auto">
      ...
      >
        <div class="flex justify-center items-center w-8 sm:w-9 lg:w-10 h-8 sm:h-9 lg:h-10 bg-red11 rounded-full">
          <ion-icon :name="item.icon" class="w-4 sm:w-5 lg:w-6 h-4 sm:h-5 lg:h-6 text-white"></ion-icon>
        </div>
        <span class="flex-grow">
          <div class="text-xs sm:text-sm lg:text-base text-gray11">{{ item.label }}</div>
          <div class="text-sm sm:text-base lg:text-lg text-mauve12">{{ item.value }}</div>
        </span>
        <span 
